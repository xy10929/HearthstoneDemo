...
public class TargetSelector : MonoBehaviour
{
    ...
    // save selected card into selectedCardDisplay and set currentState
    // Idle -> State 1 or 2
    public void BeginCardSelection(CardDisplay cardDisplay)
    {
        ...
        {
            currentState = SelectionState.SpellTargeting;
        ...
        {
            currentState = SelectionState.MinionSummoning;
    ...
    // save selected minion into selectedAttackerMinion and set currentState
    // Idle -> State 3
    public void TrySelectAttacker(Minion minion)
    {
        ...
        currentState = SelectionState.AttackerSelected;
    ...
    // State 1 -> Resolving (-> Idle)   (click spell card and then) select target
    // State 3 -> Resolving (-> Idle)   (click minion instance and then) select attack target
    public void TrySelectTarget(ITargetable target)
    {
        ...
        {
            currentState = SelectionState.Resolving;
    ...
    // State 2 -> Resolving (-> Idle)    (click minion card and then) click board to summon
    public void TrySummonSelectedMinion()
    {
        if (currentState != SelectionState.MinionSummoning)
        {
            return;
        }
